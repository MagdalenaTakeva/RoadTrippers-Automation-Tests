...
class LoginPage(BasePage):
    ...
    def discover_card_btn_is_displayed(self)-> bool:
        """
        Check if the Discover Card button is visible on the Trip Planner page
        (used to confirm successful navigation after login).
        ...
        Steps to reproduce manually:
            ...
            2. Observe Discover Card / "Start a new trip" button visible

        Expected behavior:
            - Returns True if button visible
            - Confirms planner loaded correctly

        """
        discover_card = self.wait_for_element_visibility(self.DISCOVER_CARD_BTN, timeout=BasePage.INTERACTION_TIMEOUT)
        if discover_card.is_displayed:
            self.log("Trip Planner page loaded successfully", level="debug")
            # Screenshot on success — visual confirmation of planner
            self.take_screenshot(
                self._get_current_test_name(),
                "__discover_card_visible__planner_loaded"
            )
            return True
        else:
            self.log("Trip Planner page did not load successfully", level="debug")
            # Screenshot on failure — shows what blocked the button
            self.take_screenshot(
                self._get_current_test_name(),
                "__discover_card_not_visible__planner_failed"
            )
            return False

    def click_avatar(self) -> None:
        """
        Click the user avatar/profile button in the header (post-login).

        Current behavior:
            - Opens user menu (profile, my trips, sign out, etc.)

        Steps to reproduce manually:
            1. Log in
            2. Click avatar in top-right
            3. Observe dropdown menu opens
        ...
        avatar_btn = self.wait_for_element_to_be_clickable_with_timeout(self.AVATAR_BUTTON)
        # Screenshot before click — shows logged-in header state
        self.take_screenshot(
            self._get_current_test_name(),
            "__before_click_avatar"
        )
        avatar_btn.click()
        self.log("Avatar clicked", level="debug")
        # Screenshot after click — captures opened menu
        self.take_screenshot(
            self._get_current_test_name(),
            "__after_click_avatar__menu_open"
        )
